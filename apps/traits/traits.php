...
class traits extends app
{
	...
	{
		$this->header();
		$data['items'] = $this->m_traitModel->get_all();					
	...
	{	
		...
		$item = $this->m_traitModel->load($id, $level);
	...
	{
		$this->header();
		$items = $this->m_traitModel->search($term);
	...
	public function c_type($type)
		{		
			...
			$items = $this->m_traitModel->get_all_by_type($type);
			$this->show('traitlist', $items);
	...
	{
		$this->m_traitModel->db_update();
		$this->m_traitModel->db_update_toc();
	}
		
	public function c_aeffects($dbid, $level)
	{
		$level += 3;
		$effects = $this->m_traitModel->get_effects($dbid, $level);
	...
	public function getImgData($dbid, $level)
	{
		...
		$tempitem = $this->m_traitModel->load($dbid, $level);
