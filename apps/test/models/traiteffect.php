	...
	public function __toString()
	{
		...
		else
		{
			...
			if($this->subtype == "WorkRate")
			{
				...
				else if($this->action == "FishGather")
				{
				...
				else if($this->action == "Build" && $this->unittype != "Building")
				{
					$string.="{$this->translateUnitType()} Building Speed";
				}
				
				else if ($this->action == "SelfHeal")
				{
		...
		if($this->subtype != "ActionEnable")
		{
			if($this->subtype == 'WorkRate' && $this->action == "Convert")
				$amountLeveled = $this->amountLeveled($this->level, true);
